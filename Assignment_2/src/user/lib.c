...
// message queue system call wrappers

int create_msg_queue()
{
	return _syscall0(SYSCALL_CREATE_MSG_QUEUE);
}

int get_member_info(int fd, struct msg_queue_member_info *info)
{
	return _syscall2(SYSCALL_GET_MEMBER_INFO, fd, (u64)info);
}

int msg_queue_send(int fd, struct message *msg)
{
	return _syscall2(SYSCALL_MSG_QUEUE_SEND, fd, (u64)msg);
}

int msg_queue_rcv(int fd, struct message *msg)
{
	return _syscall2(SYSCALL_MSG_QUEUE_RCV, fd, (u64)msg);
}

int get_msg_count(int fd)
{
	return _syscall1(SYSCALL_GET_MSG_COUNT, fd);
}

int msg_queue_block(int fd, int pid)
{
	return _syscall2(SYSCALL_MSG_QUEUE_BLOCK, fd, pid);
}

int msg_queue_close(int fd)
{
	return _syscall1(SYSCALL_MSG_QUEUE_CLOSE, fd);
}
