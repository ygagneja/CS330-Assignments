...
{
	return _syscall4(SYSCALL_SENDFILE, outfd, infd, (u64)offset, count);
}
...
int become_debugger()
{
	return _syscall0(SYSCALL_BECOME_DEBUGGER);
}

int set_breakpoint(void *addr)
{
	return _syscall1(SYSCALL_SET_BREAKPOINT, (u64)addr);	
}

int remove_breakpoint(void *addr)
{
	return _syscall1(SYSCALL_REMOVE_BREAKPOINT, (u64)addr);
}

int enable_breakpoint(void *addr)
{
	return _syscall1(SYSCALL_ENABLE_BREAKPOINT, (u64)addr);
}

int disable_breakpoint(void *addr)
{
	return _syscall1(SYSCALL_DISABLE_BREAKPOINT, (u64)addr);
}

int info_breakpoints(struct breakpoint *bp)
{
	return _syscall1(SYSCALL_INFO_BREAKPOINTS, (u64)bp);
}

int info_registers(struct registers *reg)
{
	return _syscall1(SYSCALL_INFO_REGISTERS, (u64)reg);
}

int backtrace(void *bt_buf)
{
	return _syscall1(SYSCALL_BACKTRACE, (u64)bt_buf);
}

s64 wait_and_continue()
{
	return _syscall0(SYSCALL_WAIT_AND_CONTINUE);
}

int test_breakpoints()
{
	return _syscall0(SYSCALL_TEST_BREAKPOINTS);
}
