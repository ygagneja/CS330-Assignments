...
long make_hugepage(void *addr, u32 length, u32 prot, u32 force)
{
	return _syscall4(SYSCALL_MAKE_HUGEPAGE, (u64)addr, length, prot, force);
}

int break_hugepage(void *addr, u32 length)
{
	return _syscall2(SYSCALL_BREAK_HUGEPAGE, (u64)addr, length);
}
